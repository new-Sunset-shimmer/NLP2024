...
class LSTM:
    ...
    def backward(self):
        ...
        h = cp.zeros((self.dim,self.batchs))
        c = cp.zeros((self.dim,self.batchs))
        temp_E = cp.zeros_like(self.weight['E'])
        ...
        del temp_E, temp_Uo, temp_Wo, temp_Uf, temp_Wf, temp_Ug, temp_Wg, temp_Ui, temp_Wi, temp_V
    def _clipping(self,x):
        return np.clip(x, -1, 1, out=x)
    def _test_run(self,x_ground):
        h = cp.zeros((self.dim,self.batchs))
        c = cp.zeros((self.dim,self.batchs))
        valid_count = 0
        total = 0
        for i in tqdm(range(self.max_len-2)):
            x = x_ground[:,i]
            x, forbackward= self.embed.forward(x,self.weight['E'])
            h, c, forbackward = self.hiddenstate.forward(x,h,c,self.weight)
            y = x_ground[:,i+1]
    ...
    def test(self,test_set):
        ...
        for sentences_idx in tqdm(range(self.batchs,int(len(self.sentences)),self.batchs)):
            ...
            x = x_list
            before_idx = sentences_idx
            temp_valid_count,temp_total = self._test_run(x)
    ...
    def _splitter_x(self, x):
        ...
        for i in range(1,self.max_len):
            if i < len(x):
                try:
                    result.append(self.dict[x[i]])
                except:
                    result.append(self.dict["<UKN>"])
            else:
                result.append(self.dict["<PAD>"])
        result.append(self.dict["<END>"])
        return result
    def _splitter_y(self, x):
        ...
        return result
    def save_model(self):
        with open("model_LSTMLM.pt","wb") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-batchs", "--batchs", type=int, default=64, action="store")
    parser.add_argument("-lr", "--lr", type=float, default=0.1, action="store")
    parser.add_argument("-train_set", "--train_set", default="raw_train.txt", action="store")
    parser.add_argument("-test_set", "--test_set", default="raw_train.txt", action="store")
    parser.add_argument("-model", "--model", default="model_LSTMLM.pt", action="store")
    ...
    if args.gpu == True:
        ...
        import numpy as cp
    if args.train == True:
        lstm = LSTM(load_file(args.train_set),args.epochs,args.batchs,args.lr,args.dim)
        lstm.train()
        lstm.save_model()
    elif args.test == True:
        lstm = LSTM()
